    ...
    def from_pretrained(
        ...
        **from_pretrained_kwargs,
        ...
        if hf_model is not None:
            ...
            '''
        Jiahai hacky patch for tulu
        '''
            assert not (
                load_in_4bit and (("llama" not in model_name.lower()) and ("qwen" not in model_name.lower()))
        ...
        Jiahai hacky patch for tulu
        '''
        if hf_model is not None and ('llama' in model_name or 'gemma' in model_name or 'Qwen'in model_name):
            if cfg.d_vocab != hf_model.model.embed_tokens.num_embeddings:
                print(f'Warning: incompatible vocabulary size')
                print(f'Setting vocab from {cfg.d_vocab} to {hf_model.model.embed_tokens.num_embeddings}')
                cfg.d_vocab = hf_model.model.embed_tokens.num_embeddings
                cfg.d_vocab_out = hf_model.model.embed_tokens.num_embeddings
        '''
        End Jiahai's hacky patch
        '''
